...
class OutlierEvaluator(Evaluator):
    ...
    def cosine(self,vector1, vector2):
        ...
        module_vector_1 = self.module(vector1)
        if module_vector_1 == 0.0: return 0.0
        module_vector_2 = self.module(vector2)
        if module_vector_2 == 0.0: return 0.0
        return self.scalar_prod(vector1, vector2) / (self.module(vector1) * self.module(vector2))


    def pairwisesimilarities_cluster(self,setElementsCluster, input_vectors):
        ...
        for element_1 in setElementsCluster:
            for element_2 in setElementsCluster:
                if element_1 != element_2:
                    dict_sim[element_1 + " " + element_2] = self.cosine(input_vectors[element_1], input_vectors[element_2])
        return dict_sim


    def compose_vectors_multiword(self,multiword,input_vectors,dimensions):
    ...
    def preprocess(self,vectors, set_words):
        # Reads input vectors file and stores the vectors of the words occurring in the dataset in a dictionary
        print("Preprocessing the vector file for Outlier detection test")
        set_vectors = {}
        dimensions=len(list(vectors.values())[0])
        ...
        for word in set_words:
            if word not in set_vectors:
                set_vectors[word] = self.compose_vectors_multiword(word,vectors, dimensions)
        return set_vectors
    def check(self,cluster, input_vectors):
    ...
    def run(self,input_vectors, dataset):
        ...
        for cluster in dataset.clusters :# 8 different files
            ...
            ischeck,clusternew=self.check(cluster,input_vectors)
            if (ischeck):
            ...
            dictSim = self.pairwisesimilarities_cluster(cluster.elements, input_vectors)
            ...
            for outlier in cluster.outliers: # one outlier has 8 elements
                ...
                for element_cluster_1 in cluster.elements: # 8 same group of elements
                   # print(element_cluster_1, outlier)
                    sim_outlier_element = self.cosine(input_vectors[element_cluster_1], input_vectors[outlier]) # cosine similarity between outlier and cluster
    ...
    def process(self, vectors: dict):
        ...
        dataset.readDataset()
        input_vectors = self.preprocess(vectors, dataset.setWords)
        out = self.run(input_vectors,dataset)
        return out
